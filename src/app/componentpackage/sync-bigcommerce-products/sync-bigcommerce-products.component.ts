...
@Component({
  selector: 'app-sync-bigcommerce-products',
  templateUrl: './sync-bigcommerce-products.component.html',
  styleUrls: ['./sync-bigcommerce-products.component.css']
})
export class SyncBigcommerceProductsComponent implements OnInit {

  Tag = "SyncBigcommerceProductsComponent"
  newrray: Array<any>
  ...
  checkLoginStatus() {
    ...
    if (loginstatus) {
      ...
      if (this.accountType == this.urlaccountType) {
        this.activatedRouter.params.subscribe(params => {
          this.keyPublicId = params['bigcommerceKey']
  ...
  onResponseGetWooCommerceKeyDetailByIdInterface(status: any, keyDataList: any) {
    ...
    switch (status) {
      case "1":
        ...
        this.auth_token = keyDataList.accessToken
        ...
        CommonMethods.showconsole(this.Tag, "Domain:- " + this.auth_token)
  ...
  syncProductsFromWooCommerce() {
    ...
    const params = new HttpParams()
      ...
      .set("Auth", MyCookies.getAuthorization(this.cookiesService))
      .set("userId", MyCookies.getUserId(this.cookiesService))
      .set("sessionToken", MyCookies.getSessionTime(this.cookiesService))
      .set('accountType', MyCookies.getAccountType(this.cookiesService))
      .set('x_auth_client', this.consumer_key)
      .set('x_auth_token', this.auth_token)
      .set("domain", this.domain)
      .set("page", "" + this.pagenumber)
      .set('limit', "" + this.pagepercount)

    this.httpClient.post(MyConstants.BASEURL + "getProductListFromBigCommerce.php", params)
      .subscribe((responseData) => {
        CommonMethods.showconsole(this.Tag, "Response Data :- " + responseData['status'])
        let status = responseData['status']
        switch (status) {
          case "1":

            
            this.dataSource =responseData['productList']
            CommonMethods.showconsole(this.Tag,"Show Product Image :- "+JSON.stringify( this.dataSource))
          ...
          case "0":
            ...
            CommonMethods.showconsole(this.Tag, "Show Error 0 :- " + responseData['message'])
            break;
          case "2":
            CommonMethods.showconsole(this.Tag, "Show error 2 :- " + responseData["errorData"])
          default:
            alert('Error Occured, Please try again');
            break;
        }
  ...
  addTowebsite() {
    if (this.validation()) {
      CommonWebApiService.addProductsFromBigCommerce(this.spinner, this.httpClient, this.cookiesService, this.snackBar, this, this.selectedListItem,this.consumer_key,this.auth_token,this.domain)
