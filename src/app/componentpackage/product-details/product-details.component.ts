  ...
  onResponseGetProductDetails(status: string, apiResponse: any) {
    switch (status) {
      case "1":
        ...
        for (var i = 0; i < this.productDetailsModal.productadditionOptionList.length; i++) {
          this.UnitType = this.productDetailsModal.productadditionOptionList[i].unitType
        }


        CommonMethods.showconsole(this.Tag, "Product Image Price  :- " + JSON.stringify(this.productDetailsModal.productadditionOptionList))
        break;
      case "10":
