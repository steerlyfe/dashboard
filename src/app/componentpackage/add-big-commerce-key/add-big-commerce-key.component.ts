...
import { AddWooCommerceKey, GetWooCommerceKeyDetailById } from 'src/app/CommonMethods/CommonInterface';

@Component({
  selector: 'app-add-big-commerce-key',
  templateUrl: './add-big-commerce-key.component.html',
  styleUrls: ['./add-big-commerce-key.component.css']
})
export class AddBigCommerceKeyComponent implements OnInit,GetWooCommerceKeyDetailById,AddWooCommerceKey {
  Tag="AddBigCommerceKeyComponent"
  ...
  constructor(public spinner: NgxSpinnerService, public router: Router, public activatedRouter: ActivatedRoute,
    ...
    public location:Location) {
      ...
      this.urlaccountType=""
      this.keypublicid=""
    this.addeCommerceKey = new AddECommerceKey("", "", "","bigCommerce","")
 ...
 /**
  ...
  onResponseGetWooCommerceKeyDetailByIdInterface(status: any, keyDataList: any) {
    ...
    switch (status) {
      case "1":
        ...
        this.addeCommerceKey.access_token = keyDataList.accessToken
         CommonMethods.showconsole(this.Tag,"Key u[date:- "+JSON.stringify(keyDataList))
        break;
      case "10":
  ...
  validation() {
    ...
    else if (this.addeCommerceKey.access_token.trim().length == 0) {
      CommonMethods.showErrorDialog(this.snackBar, "Enter  access token ")
      return false
    } 
